...
statement
    ...
    | SHOW TABLES ((FROM | IN) qualifiedName)?
        (LIKE pattern=string_ (ESCAPE escape=string_)?)?                 #showTables
    | SHOW SCHEMAS ((FROM | IN) identifier)?
        (LIKE pattern=string_ (ESCAPE escape=string_)?)?                 #showSchemas
    | SHOW CATALOGS (LIKE pattern=string_)?                             #showCatalogs
    ...
    | SHOW FUNCTIONS
        (LIKE pattern=string_ (ESCAPE escape=string_)?)?                 #showFunctions
...
columnDefinition
    : identifier type_ (NOT NULL)? (COMMENT string_)? (WITH properties)?
    ;
...
primaryExpression
    ...
    | DOUBLE_PRECISION string_                                                             #typeConstructor
...
timeZoneSpecifier
    ...
    | TIME ZONE string_    #timeZoneString
    ;
...
interval
    : INTERVAL sign=(PLUS | MINUS)? string_ from=intervalField (TO to=intervalField)?
    ;
